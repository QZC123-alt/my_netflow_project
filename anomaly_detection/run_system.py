...
# 如果run_system.py不在根目录，改成：PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))

# 直接指向项目根目录的data和models
DATA_DIR = os.path.join(PROJECT_ROOT, "data")       # 根目录/data
MODEL_DIR = os.path.join(PROJECT_ROOT, "models")     # 根目录/models
...
SCALER_PATH = os.path.join(MODEL_DIR, "scaler.pkl")                      # 根目录/models/scaler.pkl
...
def load_data(dataset_type="merge"):
    ...
    if dataset_type in ["kdd", "merge"]:
        ...
        try:
            ...
            logging.info(f"KDD加载完成：训练集{len(kdd_train)}行，标签类型={kdd_train[LABEL_COL].dtype}")
    ...
    if dataset_type in ["cic", "merge"]:
        ...
        try:
            ...
            logging.info(f"CIC原始数据加载完成：训练集{len(cic_train_raw)}行，测试集{len(cic_test_raw)}行")
            ...
            if dataset_type == "merge":
                ...
                logging.info(f"合并后：训练集{len(train_data)}行，标签类型={train_data[LABEL_COL].dtype}")
            else:
...
def preprocess_data(train_data, test_data):
    ...
    for col in string_feature_cols:
        ...
        logging.info(f"完成{col}列编码：类别数={len(encoder.classes_)}")
    ...
    # 3. 验证标签
    logging.info(f"预处理后：训练集标签类型={train_data[LABEL_COL].dtype}，测试集标签类型={test_data[LABEL_COL].dtype}")
    train_label_vals = train_data[LABEL_COL].round(6).unique()
    test_label_vals = test_data[LABEL_COL].round(6).unique()
    ...
    logging.info(f"标签数组类型：y_train={y_train.dtype}，y_test={y_test.dtype}")
    ...
    try:
        ...
        if incremental and os.path.exists(model_path):
            ...
            if new_n_estimators > 1000:  # 防止决策树过多导致内存溢出
                logging.warning(f"增量训练后决策树数量{new_n_estimators}过大，限制为1000")
                new_n_estimators = 1000
            model.n_estimators = new_n_estimators
            logging.info(f"加载已有模型：{model_path}，增量训练（总决策树数={model.n_estimators}）")
        else:
            # 全新训练：初始化随机森林
            model = RandomForestClassifier(
                ...
                n_jobs=-1,
                verbose=1,
                max_depth=20,  # 新增：限制树深度，防止过拟合
                min_samples_split=5  # 新增：防止过拟合
            )
            logging.info(f"全新训练{dataset_type}模型：决策树数量={n_estimators}")
        ...
        try:
            metrics = {
                ...
                "召回率": recall_score(y_test, y_pred, zero_division=0),  # 新增：零除法处理
                "精确率": precision_score(y_test, y_pred, zero_division=0)
            }
        except Exception as e:
            logging.warning(f"评估指标计算失败：{e}，仅返回准确率")
        ...
        logging.error(f"模型训练失败：{str(e)}", exc_info=True)
        return False, None
...
if __name__ == "__main__":
    ...
    logging.info(f"系统环境：{platform.system()} {platform.release()}")  # 新增：打印系统信息
