...
# 架构
![image](https://gitee.com/zhexiao/mnet/raw/dev/deploy/architecture.png)

# 项目部署
整套服务项目部署流程，整套操作都在**deploy**目录下进行。
...
pm5q90eo4vsah02dv9mm74ba8 *   ubuntu              Ready               Active              Leader              18.09.6
vorniuw6o9cjpf03iedg9nfuy     ubuntu              Ready               Active                                  18.09.6
xnkk8zkn0z6lbow6bw589lskv     ubuntu              Ready               Active                                  18.09.6
```
...
# 148机器(manager)测试
$ docker run -d -it --name alpine1 --restart always --network zxnet alpine 
# 152机器worker测试
$ docker run -d -it --name alpine2 --restart always --network zxnet alpine
# 153机器worker测试
$ docker run -d -it --name alpine3 --restart always --network zxnet alpine

# 互相启动ping命令
$ docker exec -it alpine1 ping alpine2
$ docker exec -it alpine1 ping alpine3
```
**注:一定要确保几台机器互相能够ping通,否则下面的启动会出现错误。**
